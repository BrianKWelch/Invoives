...
# -----------------------------
# Backup and Restore (Manual OneDrive workflow)
# -----------------------------

def get_active_db_path() -> str:
    """
    Returns the DB path the app should be using right now.
    Uses your existing get_db_path() when available.
    """
    try:
...
def safe_replace_file(target_path: str, new_bytes: bytes) -> None:
    """
    Safely replace a file on disk.
...
def restore_db_from_upload() -> bool:
    """
    UI flow. Lets user upload a SQLite DB file and replaces the current DB.
    Returns True if restored.
    """
    st.subheader("Restore database")
    st.caption("Upload your saved data.db file from OneDrive. This replaces the current database in the app instance.")
    ...
    # Basic sanity check, prevent huge accidental uploads
    file_bytes = uploaded.getvalue()
    if not file_bytes or len(file_bytes) < 100:
        st.error("That file does not look like a valid database.")
        return False

    target = get_active_db_path()
...
def download_db_backup_ui(label: str = "Download DB backup") -> None:
    ...
    if not os.path.exists(db_path):
        st.warning("No database file found to download yet.")
        return
...
def zip_and_download_pdfs_ui() -> None:
    """
    Optional: download PDFs as a zip so you can store them in OneDrive too.
    """
    st.subheader("Download PDFs (zip)")
    st.caption("This bundles invoices, payables, and payees into a single zip for easy OneDrive backup.")

    mem = io.BytesIO()
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    zip_name = f"pdf_backup_{ts}.zip"

    try:
        with zipfile.ZipFile(mem, mode="w", compression=zipfile.ZIP_DEFLATED) as z:
            for folder in [INVOICE_DIR, PAYABLES_DIR, PAYEES_DIR]:
                if not os.path.isdir(folder):
                    continue
                for fn in os.listdir(folder):
                    full = os.path.join(folder, fn)
                    if os.path.isfile(full):
                        arc = os.path.join(folder, fn)
    ...
    st.download_button(
        ...
        key=f"download_pdfs_zip_{ts}"
    )

def show_backup_restore_panel() -> None:
    """
    Put this in your sidebar after the user is authenticated.
    """
    st.sidebar.markdown("## Backup and Restore")

    # Restore once per session
    if not st.session_state.get("db_restored_this_session", False):
        st.sidebar.info("Restore your database after login if this is a fresh app start.")
        with st.sidebar.expander("Restore DB from OneDrive file", expanded=True):
            restored = restore_db_from_upload()
            if restored:
                st.session_state["db_restored_this_session"] = True
                st.sidebar.success("Restore complete. Reloading app.")
                st.rerun()
    else:
        with st.sidebar.expander("Restore DB from OneDrive file", expanded=False):
            restored = restore_db_from_upload()
            if restored:
                st.sidebar.success("Restore complete. Reloading app.")
                st.rerun()
...
def logout_flow() -> None:
    """
    Adds a safer logout that prompts user to download DB first.
    """
    ...
    if not st.session_state["pending_logout"]:
        ...
        with col1:
            if st.button("Confirm logout", key="confirm_logout", type="primary"):
                # Clear auth and session state safely
                for k in list(st.session_state.keys()):
                    if k not in ["drive_restored", "db_restored_this_session"]:  # keep nothing important, just example
                        pass
                st.session_state["authenticated"] = False
...
def main():
    ...
    with st.sidebar:
        ...
        if migration_status == "prompt":
            ...
            col1, col2 = st.sidebar.columns(2)
            with col1:
                if col1.button("Confirm logout", key="confirm_logout", type="primary"):
                    st.session_state["authenticated"] = False
                    st.session_state["username"] = None
                    st.session_state["pending_logout"] = False
                    st.rerun()
